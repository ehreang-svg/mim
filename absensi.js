...
// Fungsi helper untuk mengambil URL API secara dinamis dari config.js
function getAbsenApiUrl() {
    return window.ABSEN_API || "";
}
...
async function submitAbsen() {
    ...
    if (!apiUrl) {
        alert("Konfigurasi API Absen belum dimuat.");
        return;
    }
    ...
    try {
        ...
        if (now.getDay() === 0) {
            alert("Hari libur (Minggu)");
            return;
        }

        const listLibur = typeof HARI_LIBUR !== "undefined" ? HARI_LIBUR : [];
        if (listLibur.includes(today)) {
        ...
        if (jenis === "MASUK" && !file) {
            alert("Foto wajib diisi untuk absen masuk");
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                try {
                    ...
                    if (jarak > 100) {
                        showLoading(false);
                        alert(`Di luar radius sekolah! Jarak Anda: ${Math.round(jarak)} meter.`);
                        return;
                    }
                    ...
                    const statusKehadiran =
                        now.getHours() > 7 || (now.getHours() === 7 && now.getMinutes() > 15)
                            ? "TERLAMBAT"
                            : "TEPAT WAKTU";
                    ...
                    showLoading(true, "Mengirim data absensi...");
                    ...
                    alert(data.message || "Proses absensi selesai.");
            ...
            (err) => {
                showLoading(false);
                alert("GPS gagal/ditolak: " + err.message);
            },
            {
...
async function loadRekap() {
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}?action=getRekap&bulan=${encodeURIComponent(bulan)}`);
        const data = await response.json();

        if (!data.status) {
            tbody.innerHTML = `<tr><td colspan="7" class="text-center">Gagal memuat data rekap</td></tr>`;
            return;
        }
...
function renderRekapTable(data) {
    ...
    if (!data || data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center">Tidak ada data rekap</td></tr>`;
        return;
    }
...
async function loadDataSiswa() {
    try {
        const apiUrl = getAbsenApiUrl();
        const res = await fetch(`${apiUrl}?action=getDataSiswa`);
...
function filterKelas() {
    ...
    dataSiswa
        ...
        .forEach((x) => {
            options += `<option value="${x.nama}">${x.nama}</option>`;
        });
...
async function loadRekapSiswa() {
    try {
        ...
        const res = await fetch(`${apiUrl}?action=getRekapSiswa&bulan=${encodeURIComponent(bulan)}`);
        const result = await res.json();
...
function gantiKelasRekap() {
    ...
    let options = '<option value="">Semua Siswa</option>';
